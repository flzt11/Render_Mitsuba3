...
def process_exr_files(dir_path, save_path, percentage):
    ...
    I45_mean = I45.mean(axis=-1)
    I90_mean = I90.mean(axis=-1)
    I135_mean = I135.mean(axis=-1)
    ...
    I0_normalized_color, I45_normalized_color, I90_normalized_color, I135_normalized_color, max_threshold = unified_normalization(
        I0, I45, I90, I135, percentage
    )

    I0_img_color = (I0_normalized_color * 65535).astype(np.uint16)
    I45_img_color = (I45_normalized_color * 65535).astype(np.uint16)
    I90_img_color = (I90_normalized_color * 65535).astype(np.uint16)
    I135_img_color = (I135_normalized_color * 65535).astype(np.uint16)
    ...
    # 使用 .mean(axis=-1) 将彩色图像转换为灰度图像
    I0_img_gray = I0_img_color.mean(axis=-1).astype(np.uint16)
    I45_img_gray = I45_img_color.mean(axis=-1).astype(np.uint16)
    I90_img_gray = I90_img_color.mean(axis=-1).astype(np.uint16)
    I135_img_gray = I135_img_color.mean(axis=-1).astype(np.uint16)
    ...
    I0_normalized, I45_normalized, I90_normalized, I135_normalized, max_threshold = unified_normalization(
        I0_mean, I45_mean, I90_mean, I135_mean, percentage
    )

    S0_new = (I0_normalized + I45_normalized + I90_normalized + I135_normalized) / 2
    S1_new = I0_normalized - I90_normalized
    S2_new = I45_normalized - I135_normalized

    S0_image = (((np.clip(I0, 0, 1) + np.clip(I45, 0, 1) + np.clip(I90, 0, 1) + np.clip(I135, 0, 1)) / 4) * 65535).astype(np.uint16)
...
if __name__ == "__main__":
    ...
    save_path = r'D:\yan\shujuji\Diffusion_dataset\processed_data_gn'
