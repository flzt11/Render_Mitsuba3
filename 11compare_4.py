...
folder4 = r'D:\yan\shujuji\Diffusion_dataset\processed_data_gn'
folder2 = r'D:\yan\shujuji\Diffusion_dataset\synthetic_data'
folder1 = r'D:\yan\shujuji\Diffusion_dataset\processed_data_origin_Stokes'
output_folder = r'D:\yan\shujuji\Diffusion_dataset\compare_4'
...
folder_name4 = os.path.basename(folder4)
...
for subfolder in subfolders:
    ...
    subfolder_path4 = os.path.join(folder4, subfolder)

    # 检查四个文件夹中是否都存在相同的子文件夹
    if os.path.exists(subfolder_path2) and os.path.exists(subfolder_path3) and os.path.exists(subfolder_path4):
        ...
        for png_file in png_files:
            ...
            img_path4 = os.path.join(subfolder_path4, png_file)

            # 检查四个文件夹中是否都存在相同的图片
            if os.path.exists(img_path2) and os.path.exists(img_path3) and os.path.exists(img_path4):
                ...
                # 创建新图像用于比较，并添加文本
                new_img = np.zeros((height + 30, width * 4, 3), dtype=np.uint8)
                new_img[30:, :width] = img1
                new_img[30:, width:width*2] = img2
                new_img[30:, width*2:width*3] = img3
                new_img[30:, width*3:width*4] = img4
                ...
                cv2.putText(new_img, subfolder + ' (' + folder_name4 + ')', (3 * width + 10, 20), font, font_scale, font_color, font_thickness, cv2.LINE_AA)
